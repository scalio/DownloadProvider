...
import static android.provider.Transfers.Impl.STATUS_BAD_REQUEST;
import static android.provider.Transfers.Impl.STATUS_CANNOT_RESUME;
import static android.provider.Transfers.Impl.STATUS_FILE_ERROR;
import static android.provider.Transfers.Impl.STATUS_HTTP_DATA_ERROR;
import static android.provider.Transfers.Impl.STATUS_SUCCESS;
import static android.provider.Transfers.Impl.STATUS_TOO_MANY_REDIRECTS;
import static android.provider.Transfers.Impl.STATUS_WAITING_FOR_NETWORK;
import static android.provider.Transfers.Impl.STATUS_WAITING_TO_RETRY;
...
public abstract class TransferThread implements Runnable {
    ...
    private final TransferInfo mInfo;
    ...
    public void run() {
        ...
        try {
            ...
            mNotifier.notifyTrasferSpeed(mInfo.mId, 0);
        }
    }
    ...
    private void checkConnectivity() throws StopRequestException {
        ...
        if (networkUsable != NetworkState.OK) {
            int status = Transfers.Impl.STATUS_WAITING_FOR_NETWORK;
            if (networkUsable == NetworkState.UNUSABLE_DUE_TO_SIZE) {
                status = Transfers.Impl.STATUS_QUEUED_FOR_WIFI;
    ...
    private void checkPausedOrCanceled(State state) throws StopRequestException {
        synchronized (mInfo) {
            if (mInfo.mControl == Transfers.Impl.CONTROL_PAUSED) {
                throw new StopRequestException(
                        Transfers.Impl.STATUS_PAUSED_BY_APP, "transfer paused by owner");
            }
            if (mInfo.mStatus == Transfers.Impl.STATUS_CANCELED || mInfo.mDeleted) {
                throw new StopRequestException(Transfers.Impl.STATUS_CANCELED, "transfer canceled");
            }
        }
    ...
    private void reportProgress(State state) {
        ...
        if (state.mCurrentBytes - state.mBytesNotified > Constants.MIN_PROGRESS_STEP &&
            ...
            values.put(Transfers.Impl.COLUMN_CURRENT_BYTES, state.mCurrentBytes);
            mContext.getContentResolver().update(mInfo.getAllTransfersUri(), values, null, null);
    ...
    private void handleEndOfStream(State state) throws StopRequestException {
        ...
        if (state.mContentLength == -1) {
            values.put(Transfers.Impl.COLUMN_TOTAL_BYTES, state.mCurrentBytes);
        }
        mContext.getContentResolver().update(mInfo.getAllTransfersUri(), values, null, null);
    ...
    private void updateDatabaseFromHeaders(State state) {
        ContentValues values = new ContentValues();
        values.put(Transfers.Impl._DATA, state.mFilename);
        if (state.mHeaderETag != null) {
            values.put(Constants.ETAG, state.mHeaderETag);
        }
        if (state.mMimeType != null) {
            values.put(Transfers.Impl.COLUMN_MIME_TYPE, state.mMimeType);
        }
        values.put(Transfers.Impl.COLUMN_TOTAL_BYTES, mInfo.mTotalBytes);
        mContext.getContentResolver().update(mInfo.getAllTransfersUri(), values, null, null);
    }
    ...
    private void notifyTransferCompleted(
        ...
        if (Transfers.Impl.isStatusCompleted(finalStatus)) {
    ...
    private void notifyThroughDatabase(
            State state, int finalStatus, String errorMsg, int numFailed) {
        ContentValues values = new ContentValues();
        values.put(Transfers.Impl.COLUMN_STATUS, finalStatus);
        values.put(Transfers.Impl._DATA, state.mFilename);
        values.put(Transfers.Impl.COLUMN_MIME_TYPE, state.mMimeType);
        values.put(Transfers.Impl.COLUMN_LAST_MODIFICATION, mSystemFacade.currentTimeMillis());
        values.put(Transfers.Impl.COLUMN_FAILED_CONNECTIONS, numFailed);
        values.put(Constants.RETRY_AFTER_X_REDIRECT_COUNT, state.mRetryAfter);

        if (!TextUtils.equals(mInfo.mUri, state.mRequestUri)) {
            values.put(Transfers.Impl.COLUMN_URI, state.mRequestUri);
        }

        // save the error message. could be useful to developers.
        if (!TextUtils.isEmpty(errorMsg)) {
            values.put(Transfers.Impl.COLUMN_ERROR_MSG, errorMsg);
        }
        mContext.getContentResolver().update(mInfo.getAllTransfersUri(), values, null, null);
    }
     ...
     * {@link android.provider.Transfers.Impl#STATUS_WAITING_TO_RETRY}.
     */
